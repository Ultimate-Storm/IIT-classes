...
(defn deletef
  ...
  (cond
    (empty? xx) nil
    (= elt (:car xx)) (:cdr xx)
    :else (Cons. (:car xx) (deletef elt (:cdr xx )
        ))))
(defn delete [elt xx]
    (cond
      (= (List. (deletef elt (:data xx)) (:size xx)) xx) xx 
      :else (List. (deletef elt (:data xx)) (- (:size xx) 1))))
...
(defn deletef-all
  ...
  (cond
    (nil? xx) nil
    (= elt (:car xx)) (deletef-all elt (:cdr xx))
    :else (Cons. (:car xx) (deletef-all elt (:cdr xx)))))
(defn difference [elt xx]
    (loop [xx (:data xx)
            s 0] 
            (if(not(= xx (deletef elt xx)))
                    (recur (deletef elt xx) (+ s 1))s)))
(defn delete-all [elt xx]
    (cond
      (= (List. (deletef elt (:data xx)) (:size xx)) xx) xx
      :else (List. (deletef-all elt (:data xx)) (- (:size xx) (difference elt xx)))))
